...
describe('ed25519', () => {
  ...
  function to32Bytes(numOrStr) {
    ...
    return hexToBytes(hex2.padStart(64, '0'));
  }
    ...
    fc.assert(
      fc.property(
        ...
        (message, privateKey) => {
          ...
          eql(publicKey.length, 32);
          eql(signature.length, 64);
          eql(ed.verify(signature, to32Bytes(message), publicKey), true);
        }
      ),
      { numRuns: 5 }
    );
  ...
  describe('BASE_POINT.multiply()', () => {
    ...
    should('create right publicKey without SHA-512 hashing TEST 1', () => {
      ...
      eql(publicKey.toHex(), '0f3b913371411b27e646b537e888f685bf929ea7aab93c950ed84433f064480d');
    });
    should('create right publicKey without SHA-512 hashing TEST 2', () => {
      ...
      eql(publicKey.toHex(), 'ad545340b58610f0cd62f17d55af1ab11ecde9c084d5476865ddb4dbda015349');
    });
    should('create right publicKey without SHA-512 hashing TEST 3', () => {
      ...
      eql(publicKey.toHex(), 'e097c4415fe85724d522b2e449e8fd78dd40d20097bdc9ae36fe8ec6fe12cb8c');
    });
    should('create right publicKey without SHA-512 hashing TEST 4', () => {
      ...
      eql(publicKey.toHex(), 'f12cb7c43b59971395926f278ce7c2eaded9444fbce62ca717564cb508a0db1d');
    });
  ...
  should('rfc8032 vectors/should create right signature for 0x9d and empty string', () => {
    ...
    eql(hex(publicKey), 'd75a980182b10ab7d54bfed3c964073a0ee172f3daa62325af021a68f707511a');
    eql(
      hex(signature),
  ...
  should('rfc8032 vectors/should create right signature for 0x4c and 72', () => {
    ...
    const message = '72';
    const signature = ed.sign(message, privateKey);
    eql(hex(publicKey), '3d4017c3e843895a92b70aa74d1b7ebc9c982ccf2ec4968cc0cd55f12af4660c');
    eql(
      hex(signature),
  ...
  should('rfc8032 vectors/should create right signature for 0x00 and 5a', () => {
    ...
    const signature = ed.sign(message, privateKey);
    eql(hex(publicKey), '77d1d8ebacd13f4e2f8a40e28c4a63bc9ce3bfb69716334bcb28a33eb134086c');
    eql(
      hex(signature),
  ...
  should('rfc8032 vectors/should create right signature for 0xf5 and long msg', () => {
    ...
    const signature = ed.sign(message, privateKey);
    eql(hex(publicKey), '278117fc144c72340f67d0f2316e8386ceffbf2b2428c9c51fef7c597f1d426e');
    eql(
      hex(signature),
  ...
  describe('ZIP215', () => {
    ...
    should('disallow sig.s >= CURVE.n', () => {
      ...
      throws(() => {
        eql(ed.verify(sig, 'deadbeef', Point.BASE), false);
      });
      eql(ed.verify(sig, 'be'.repeat(64), Point.BASE.toRawBytes()), false);
    });
  });
